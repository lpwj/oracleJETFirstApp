...
define([
	...
	function (
		...
		CustomerViewModel.prototype._initAllObservables = function () {
			...
			// dataprovider
			this.inputCountryDataProvider = ko.observable(new ArrayDataProvider([
				{
					value: 1,
					label: 'Portugal'
				},
			...
			this.inputStateDataProvider = ko.observable(new ArrayDataProvider([], {
				keyAttributes: 'value',
			}));
		};
		...
		CustomerViewModel.prototype._onInputCountryValueChanged = function (event) {
			...
			if (value) {
				...
				this.inputStateDataProvider(new ArrayDataProvider(statesArray, {
					keyAttributes: 'value',
				}));
			}
			else {
